...
/** Provide a "Lane" layered in a pipeline Node. For instance, execution lane 0 and lane 1 on
  * execute node 3 would be two lanes layered in the execute node 3.
  */
trait NodeLaneApi {
  def nodeLink: Node
  def laneName: String
  ...
  def apply[T <: Data](that: Payload[T], subKey: Any): T =
    _c.apply(that, laneName + "_" + subKey.toString)
...
//    override def isCancel: Bool = node.isCancel
  ...
  implicit def bundlePimper[T <: Bundle](stageable: Payload[T]): BundlePimper[T] =
    new BundlePimper[T](this(stageable))
