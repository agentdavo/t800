...
class RegFilePlugin extends FiberPlugin with RegfileService {
  ...
  override def read(reg: RegName.C, processId: UInt, shadow: Boolean): Bits = B(0, rfSpec.width bits)
  ...
  override def readStatusBit(field: StatusRegBits => Bool, processId: UInt, shadow: Boolean): Bool = False
