...
/** T9000 Clock Domain Integration
  *
  * Integrates the T9000 multi-clock architecture with PMI and VCP plugins, implementing proper
  * clock domain crossings and synchronized operation.
  */
class T9000ClockIntegration extends Component {
  ...
  val vcpIntegration = new Area {
    ...
    val packetRouter = clockManager.systemDomain {
      new Area {
        val routingTable =
          Vec(Reg(UInt(3 bits)) init 0, 256) // Virtual channel to physical link mapping
  ...
  val timerIntegration = clockManager.timerDomain {
    new Area {
      ...
      val timer1usSync = T9000ClockDomains.CrossDomain.pulseSynchronizer(
        clockManager.timerDomain,
        clockManager.systemDomain,
        timer1usInterrupt
      )
...
/** T9000 Clock Domain Service Plugin Provides clock domain management as a service to other plugins
  *
  * NOTE: This plugin has been extracted to a proper plugin structure at:
  * src/main/scala/transputer/plugins/clock/ClocksPlugin.scala
  *
  * The version below is kept as an example of integration with PMI and VCP. For general use, prefer
  * the extracted ClocksPlugin.
  */
class T9000ClockServicePlugin extends FiberPlugin {
  ...
  during setup new Area {
    println(s"[${T9000ClockServicePlugin.this.getDisplayName()}] Setup starting...")
    println(s"[${T9000ClockServicePlugin.this.getDisplayName()}] Setup complete")
  }

  during build new Area {
    println(s"[${T9000ClockServicePlugin.this.getDisplayName()}] Build starting...")
    ...
    println(s"[${T9000ClockServicePlugin.this.getDisplayName()}] Build complete")
  }
}
