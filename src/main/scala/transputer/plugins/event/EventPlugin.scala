...
class EventPlugin extends FiberPlugin {
  ...
  private val eventMaskReg = Reg(Bits(32 bits)) init 0
  private val eventStatusReg = Reg(Bits(32 bits)) init 0
  private val eventEnabledReg = Reg(Bool()) init False
  private val interruptEnabledReg = Reg(Bool()) init False

  // Event Channel Management
  private val eventChannelReady = Vec(Reg(Bool()) init False, MAX_EVENT_CHANNELS)
  private val eventChannelData = Vec(Reg(Bits(32 bits)) init 0, MAX_EVENT_CHANNELS)
  private val eventChannelEnabled = Vec(Reg(Bool()) init False, MAX_EVENT_CHANNELS)

  // Interrupt System Registers
  private val interruptRequestReg = Reg(Bool()) init False
  private val interruptAcknowledgeReg = Reg(Bool()) init False
  private val interruptVectorReg = Reg(Bits(8 bits)) init 0
  private val interruptPriorityReg = Reg(UInt(3 bits)) init 0

  // Error Management Registers
  private val errorCodeReg = Reg(Bits(16 bits)) init 0
  private val errorAddressReg = Reg(UInt(32 bits)) init 0
  private val errorFlagsReg = Reg(Bits(16 bits)) init 0
  private val errorActive = Reg(Bool()) init False

  // Timer Event Support
  private val timerExpiredFlags = Vec(Reg(Bool()) init False, 2) // T9000 has 2 timers
  private val timerEventEnabled = Vec(Reg(Bool()) init False, 2)

  // Process Event Support
  private val processReadyReg = Reg(Bool()) init False
  private val processTerminatedReg = Reg(Bool()) init False
  private val processEventsEnabled = Reg(Bool()) init False
