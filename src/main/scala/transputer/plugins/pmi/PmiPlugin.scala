...
class PmiPlugin(
  ...
  during setup new Area {
    ...
    require(Set("sram", "dram", "flash").contains(timingPreset), s"Unknown timing preset: $timingPreset")
    require(maxBurstLength > 0 && maxBurstLength <= 256, "Burst length must be 1-256")
    
    println(s"[${getDisplayName()}] Configuration:")
  ...
  during build new Area {
    ...
    val dmaQueue = if (enableDma) Some(StreamFifo(
      dataType = PmiDmaCmd(),
    ...
    addService(new PmiService {
      ...
      override def setTimingParameters(access: UInt, setup: UInt, hold: UInt, recovery: UInt): Unit = {
