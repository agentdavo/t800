...
/** T9000 Memory Protection Plugin implementing logical-to-physical address translation
  * and access checking for P-processes.
  *
  * Features:
...
class MemoryProtectionPlugin extends FiberPlugin {
  ...
  during setup new Area {
    ...
    addService(new MemoryProtectionService {
      ...
      override def configureRegion(regionId: UInt, base: UInt, size: UInt, permissions: Bits): Unit = {
  ...
  during build new Area {
    ...
    println(s"[${MemoryProtectionPlugin.this.getDisplayName()}] Memory protection hardware configured")
    println(s"[${MemoryProtectionPlugin.this.getDisplayName()}] - 4 configurable memory regions")
    println(s"[${MemoryProtectionPlugin.this.getDisplayName()}] - Logical to physical address translation")
    println(s"[${MemoryProtectionPlugin.this.getDisplayName()}] - Read/write/execute permission checking")
  ...
  /** Address translation function for logical to physical mapping.
    * 
    * T9000 memory management combines low-order logical address bits with
    * high-order physical base bits from the appropriate region register.
    */
  ...
  /** Configure a memory region for the current P-process.
    ...
    * @param permissions [2:execute, 1:write, 0:read]
    */
  ...
  /** Enter protected mode for P-process execution.
    *
    * @param regions 4 memory regions to configure
    * @param supervisorReturn L-process return address
    */
