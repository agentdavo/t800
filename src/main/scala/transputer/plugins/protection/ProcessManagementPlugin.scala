...
class ProcessManagementPlugin extends FiberPlugin {
  ...
  during build new Area {
    ...
    println(
      s"[${ProcessManagementPlugin.this.getDisplayName()}] Process management hardware configured"
    )
  ...
  /** Handle system call from P-process.
    *
    * P-processes use syscall instruction to request services from supervisor. The request is passed
    * to the supervisor L-process for handling.
    */
