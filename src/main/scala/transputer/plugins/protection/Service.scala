...
object ProtectionTypes {
  ...
  object ErrorType extends SpinalEnum {
    val MEMORY_VIOLATION, ILLEGAL_INSTRUCTION, SYSCALL, STACK_OVERFLOW, ARITHMETIC_ERROR, TIMEOUT,
      COMMUNICATION_ERROR = newElement()
  }
...
/** Service interfaces for T9000 protection and error handling system.
  *
  * These services define the contracts between protection plugins and provide clean interfaces for
  * privilege checking, memory protection, process management, and event handling.
  */
...
trait MemoryProtectionService {
  ...
  /** Translate logical address to physical address with permission checking.
    *
    * @param logical
    *   Logical address from instruction/data access
    * @param accessType
    *   Access type: [2:execute, 1:write, 0:read]
    * @return
    *   Physical address
    */
  ...
  /** Configure a memory region for current P-process.
    ...
    * @param permissions
    *   Permission bits [2:execute, 1:write, 0:read]
    */
  ...
  /** Enter protected mode with specified memory regions.
    *
    * @param regions
    *   Array of 4 memory regions
    * @param supervisor
    *   L-process supervisor address
    */
  ...
  /** Get information about last memory violation.
    *
    * @return
    *   (violationAddress, violationType)
    */
...
trait ProcessManagementService {
  ...
  /** Create a new P-process under supervisor control.
    *
    * @param supervisor
    *   Address of supervisor L-process
    * @return
    *   Process ID
    */
  ...
  /** Get current process type.
    *
    * @return
    *   Current process type (NORMAL, L_PROCESS, P_PROCESS)
    */
  ...
  /** Check if instruction is privileged.
    ...
    *   Instruction opcode
    * @return
    *   True if instruction requires supervisor privileges
    */
...
trait EventChannelService {
  ...
  /** Configure an Event channel for input or output.
    ...
    *   Channel index (0-3)
    * @param direction
    *   INPUT (interrupt) or OUTPUT (handshake)
    * @param processId
  ...
  /** Get interrupt request status.
    *
    * @return
    *   True if any event channel has pending interrupt
    */
...
trait PrivilegeService {
  ...
  /** Check if instruction is allowed at current privilege level.
    ...
    * @return
    *   True if instruction is allowed
    */
  ...
  /** Check if opcode is privileged.
    ...
    *   Instruction opcode
    * @return
    *   True if instruction is privileged
    */
  ...
  /** Generate trap for illegal instruction.
    ...
    * @param addr
    *   Address where illegal instruction occurred
    */
...
trait ErrorHandlingService {
  ...
  /** Check if error handling is available.
    *
    * @return
    *   True if error can be handled locally
    */
  ...
  /** Get error handler address for current process.
    *
    * @return
    *   Error handler address or 0 if none
    */
  ...
  /** Set global error handler.
    *
    * @param handler
    *   Address of global error handling code
    */
