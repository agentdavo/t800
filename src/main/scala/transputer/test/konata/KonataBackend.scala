...
/** Konata log format backend for T9000 pipeline visualization.
  *
  * Implements Konata format version 4 for visualizing instruction flow
  * through the T9000's 5-stage pipeline:
  * - Fetch/Group (F)
  * - Local/Decode (D) 
  * - Address/Cache (A)
  * - Execute (X)
...
class KonataBackend(file: File) {
  ...
  case class InsnStartEvent(cycle: Long, pc: Long, mnemonic: String, threadId: Int = 0) extends KonataEvent
  case class StageStartEvent(cycle: Long, insnId: Long, stage: String, lane: Int = 0) extends KonataEvent
  case class StageEndEvent(cycle: Long, insnId: Long, stage: String, lane: Int = 0) extends KonataEvent
  ...
  case class CommentEvent(cycle: Long, insnId: Long, text: String, mouseOver: Boolean = true) extends KonataEvent
