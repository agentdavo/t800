...
object BootloadTraceSim {
    ...
    try {
      while (!halted && cycle < maxCycles && memory.contains(iptr)) {
        ...
        mnemonic match {
          case "IN" =>
            println(
              s"-I-TRACE: In(1): Channel=#${areg.toHexString}, to memory at #${breg.toHexString}, length #${creg}"
            )
            writer.println(
              s"-I-TRACE: In(1): Channel=#${areg.toHexString}, to memory at #${breg.toHexString}, length #${creg}"
            )
  ...
  def hardwareSimulation(args: Array[String]): Unit = {
    val hexFile = if (args.nonEmpty) args(0) else "scripts/hex/bootload.hex"
    val traceFile =
      if (args.length > 1) args(1) else "scripts/test_reports/bootload_hardware_trace.txt"
    ...
    compiled.doSim { dut =>
      ...
      writer.println(
        "-Cycle--IPtr------Code--------Mnemonic------------AReg-----BReg-----CReg-----WPtr---"
      )
      writer.println("-" * 80)
      ...
      try {
        ...
        while (cycle < maxCycles && !dut.io.halted.toBoolean) {
          ...
          if (iptr != lastIptr || cycle % 100 == 0) {
            ...
            val bytes = (0 to 3).map(i => memoryMap.getOrElse(iptr + i, 0)).toList
            val (mnemonic, operand, length) = decodeInstruction(bytes, iptr)
            val instrBytes = bytes.take(length)
            val codeStr = instrBytes.map(b => f"$b%02X").mkString(" ")

            val traceLine =
              f"$cycle%6d  ${iptr}%08X  $codeStr%-12s  $mnemonic%-8s $operand%-8s  ${areg}%08X ${breg}%08X ${creg}%08X ${wptr}%08X"
