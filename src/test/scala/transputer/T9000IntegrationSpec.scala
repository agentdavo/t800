...
/** Test specification for T9000 Complete System Integration Tests the full T9000 system with
...
class T9000IntegrationSpec extends AnyFunSuite {
    ...
    SimConfig.compile(new IntegrationTestDut).doSim { dut =>
      ...
      for (cycle <- 0 until 100) {
        ...
        if (cycle % 20 == 0) {
          ...
          assert(
            cmdReady == true || cmdReady == false,
            s"cmd.ready should be stable at cycle $cycle"
          )
          assert(
          ...
          assert(
            rspReady == true || rspReady == false,
            s"rsp.ready should be stable at cycle $cycle"
          )
        }
      }
    }
  ...
  test("T9000Integration should handle system stress test") {
    SimConfig.compile(new IntegrationTestDut).doSim { dut =>
      ...
      assert(
        errorCount < stressCycles / 10,
        s"Too many errors during stress test: $errorCount/$stressCycles"
      )
      assert(
        true,
        s"System stress test completed with $errorCount errors out of $stressCycles cycles"
      )
    }
  }
}
