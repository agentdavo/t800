...
/** Test specification for T9000 Main Cache System Tests the 16KB main cache with 4-way associative
  * design
  */
    ...
    SimConfig.compile(new MainCacheTestDut).doSim { dut =>
      ...
      for (cycle <- 0 until 100) {
        ...
        assert(
          ...
          s"cmd.valid should be boolean at cycle $cycle"
        )
        assert(
          cmdReady == true || cmdReady == false,
          s"cmd.ready should be boolean at cycle $cycle"
        )
        assert(
          rspValid == true || rspValid == false,
          s"rsp.valid should be boolean at cycle $cycle"
        )
        assert(
          rspReady == true || rspReady == false,
          s"rsp.ready should be boolean at cycle $cycle"
        )
      }
    }
  }
